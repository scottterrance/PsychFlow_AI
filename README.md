...
Groq's free tier has a per-minute **tokens** limit (TPM), not just a per-minute requests limit. One full pipeline run on the default `openai/gpt-oss-120b` model can briefly exceed the 8000 TPM limit if your inputs are long.
...
**If you hit the limit too often**, switch to a model that uses fewer tokens — set `GROQ_MODEL=llama-3.3-70b-versatile` in `backend/.env` and restart `uvicorn`. Quality is still very good and you'll rarely (if ever) hit the limit. See [Groq rate limits](https://console.groq.com/docs/rate-limits).
...
| `429 Too Many Requests` (after retries) | The app already retries automatically. If it still fails, switch to `GROQ_MODEL=llama-3.3-70b-versatile` in `backend/.env`. |
